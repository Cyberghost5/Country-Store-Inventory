...
<html lang="en">
  <head>
    ...
    <title>Closing Stock - Country Store</title>
      ...
      <main class="main-content">
        ...
        <header class="topbar">
          <div class="title-block">
            <h2>Closing Stock</h2>
            <p>Record the quantity of each product remaining at the end of the day.</p>
          </div>
          @if (!$products->isEmpty() && !$alreadySaved)
            <div class="top-actions">
              <button type="submit" form="closingStockForm" class="primary-btn">
        ...
        {{-- Date selector --}}
        <form method="GET" action="{{ route('closing_stock.index') }}" class="os-date-bar">
          ...
          @if ($date !== today()->toDateString())
            <a href="{{ route('closing_stock.index') }}" class="os-today-btn">Back to Today</a>
          @endif
        </form>
        ...
        @else
          <h3 class="section-title" style="margin-bottom:14px;">
            Closing Stock for {{ \Carbon\Carbon::parse($date)->format('D, d M Y') }}
          </h3>

          @if ($alreadySaved)
            <div class="lp-success" style="margin-bottom:14px;">
              <i class="bi bi-lock-fill"></i> Closing stock has already been recorded for this day.
            </div>
          @endif

          <form method="POST" action="{{ route('closing_stock.store') }}" id="closingStockForm">
            @csrf
                ...
                <div class="os-card">
                  ...
                  <div class="os-qty-wrap">
                    <input type="number" name="stocks[{{ $product->id }}][quantity]" class="os-qty-input"
                           placeholder="Qty"
                           value="{{ $product->closing_qty_for_date ?? '' }}"
                           min="0"
                           {{ $alreadySaved ? 'readonly' : '' }} />
                  </div>
                  <textarea name="stocks[{{ $product->id }}][notes]" class="os-notes-input" rows="1"
                            placeholder="Notes (optional)"
                            {{ $alreadySaved ? 'readonly' : '' }}>{{ $product->closing_notes_for_date ?? '' }}</textarea>
                </div>
              @endforeach
            </div>
            @if (!$alreadySaved)
                ...
                <tbody>
                  @foreach ($history as $entry)
                    <tr>
                      ...
                      @if ($user->isAdmin())
                        <td>
                          <form method="POST" action="{{ route('closing_stock.destroy', $entry) }}"
