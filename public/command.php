...
$allowedCommands = [
    ...
    // Info
    'route_list'       => ['label' => 'Route List',        'cmd' => 'route:list',              'group' => 'Info',     'danger' => false, 'type' => 'artisan'],
    'about'            => ['label' => 'About',             'cmd' => 'about',                   'group' => 'Info',     'danger' => false, 'type' => 'artisan'],
    'env_show'         => ['label' => 'Environment',       'cmd' => 'env',                     'group' => 'Info',     'danger' => false, 'type' => 'artisan'],
    // Composer
    'composer_install' => ['label' => 'Composer Install',  'cmd' => 'composer install --no-interaction', 'group' => 'Composer', 'danger' => false, 'type' => 'shell'],
    'composer_dump'    => ['label' => 'Dump Autoload',     'cmd' => 'composer dump-autoload',           'group' => 'Composer', 'danger' => false, 'type' => 'shell'],
];
...
if ($isAjax && isset($_GET['run'])) {
    ...
    $meta    = $allowedCommands[$cmdKey];
    $type    = $meta['type'] ?? 'artisan';
    $t0      = microtime(true);

    if ($type === 'shell') {
        $rootDir = realpath(__DIR__ . '/../');
        chdir($rootDir);
        exec($meta['cmd'] . ' 2>&1', $outputLines, $exitCode);
        $output  = implode("\n", $outputLines);
        ...
        $status  = $kernel->call($meta['cmd']);
        $output  = trim($kernel->output());
        $elapsed = round(microtime(true) - LARAVEL_START, 3);
        echo json_encode([
            'success'   => $status === 0,
            'exit_code' => $status,
            'command'   => 'php artisan ' . $meta['cmd'],
            'output'    => $output !== '' ? $output : '(no output)',
            'elapsed'   => $elapsed . 's',
        ]);
    }
...
<title>Country Store - Command Panel</title>
<style>
...
<div class="header">
  <div class="header-title">🛠 <span>Country Store</span> Command Panel</div>
